...
> **30 秒速读**：RAG 通过"检索相关文档片段 → 注入 prompt → 模型基于片段回答"三步管道，让 Agent 能引用外部知识库而非凭记忆猜测；Agentic RAG 把检索变成工具，让 Agent 自主决定是否检索、检索几次。
> 
> **如果只记一件事**：分块策略决定检索质量——chunk_size=200-500 字符、overlap=50-100 字符是中文文档的经验值，太大太小都会出问题。
...
| 错误 | 症状 | 解决 |
|------|------|------|
| 中文没做单字拆分，直接 `text.split()` | query 和文档零词汇重叠，余弦相似度全为 0，检索返回空 | 检测连续中文字符（`'\u4e00' <= c <= '\u9fff'`），拆成单字 |
| 没设相似度阈值，无脑取 top_k | 检索到的全是无关片段，模型基于噪声回答，质量反而比不检索更差 | 设 `threshold=0.3`，低于阈值的片段不塞进 context |
| overlap 设为 0 | 关键句子正好在块边界被切断，两个块都"部分匹配"但都不完整 | overlap 设为 chunk_size 的 10%-20%（如 chunk_size=200 时 overlap=40） |
| Embedding 模型和检索时用的不一样 | 建索引用 `text-embedding-3-small`，查询用 `text-embedding-ada-002`，向量空间不同，相似度无意义 | 建索引和查询必须用同一个 Embedding 模型 |
