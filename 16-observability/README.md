...
**关键**：`payload` 必须是**结构化的**（dict / 字段固定），不能是自由文本。
`"result": "北京今天晴 25°C"` 可以，`"result": "工具返回了北京天气，是晴天25度"` 不行——后者机器解析不了。
...
看起来很少？但 Agent 一轮循环可能调 5-10 次 LLM，每天上千次查询，月账单轻松到几百美元。
**不追踪成本，你的钱包会在不知不觉中被掏空**。
...
延迟比成本更简单——每个 span 记录 `start_time` 和 `end_time`，差值就是耗时：

```python
duration_ms = (entry.end_time - entry.start_time) * 1000
```

**关键洞察**：总延迟 ≠ LLM 延迟 + 工具延迟。Agent 循环是**串行**的（调 LLM → 等结果 → 调工具 → 等结果），
所以总延迟是所有 span 耗时之和。如果某步特别慢，trace 树会一眼高亮——这是定位 P99 延迟的首选手段。
...
生产环境用 LangSmith / Langfuse 的 Web UI 看交互式 trace 树。但教学/本地调试时，
**ASCII 树是最直接的**——不依赖任何服务，终端里直接打印，立刻看到层级关系。
...
一眼看出：第 1 步花了 510ms，其中 LLM 决策 390ms（76%）、工具调用 120ms（24%）——
瓶颈在 LLM 而非工具。这就是 trace 可视化的价值。
...
**核心洞察**：LangSmith / Langfuse 的 SDK 本质上就是一个**自动版的 `TraceCollector`**——
它在你的 LLM 调用前后自动插桩（monkey-patch SDK 或用装饰器），收集本章手写的那些 span。
你理解了本章的 `TraceEntry` 结构和树渲染，打开 LangSmith 的 trace UI 就会觉得"原来是这么回事"。

### 本教程为什么不接

1. **教学原则**："先原理后工具"。自实现 trace 让你看清每一行代码在做什么，接 LangSmith 你只看到一个装饰器
2. **离线友好**：本章代码 100% 离线可跑（trace 存内存），不依赖外部服务（有些网络受限）
3. **迁移成本低**：理解原理后，接 LangSmith 就是把 `TraceCollector.add_entry()` 换成 `langfuse.span()`，5 分钟的事
...
**后果**：日志只能人眼读，无法做数据分析。Agent 上量后，你面对几十万行 print 日志，
想统计什么都统计不了。
...
- 无法做成本归因（是哪个用户/哪个功能烧的钱？）
- 无法做预算控制（"每天超过 $50 就告警"这种基础能力都没有）
...
输出标记（便于 QA grep）：
- `OUT:trace:step{N}:` — 每步 trace entry
- `OUT:cost:` — 成本计算结果
- `OUT:viz:` — ASCII 树可视化
- `OUT:log:` — 结构化日志（JSON）
