...
| 错误 | 症状 | 解决 |
|------|------|------|
| 裸 `except` 吞掉所有错误 | Agent 静默失败，日志里什么都看不到 | 捕获具体异常类型，把错误信息反馈给 Agent |
| 对认证错误也重试 | 401 错误重试 3 次，浪费时间和资源 | 用 `is_retryable()` 判断，认证/参数错误立即失败 |
| 重试时没加退避 | 限流后立刻重试，触发更严厉的封禁 | 用 `2 ** attempt` 等待，每次翻倍 |
