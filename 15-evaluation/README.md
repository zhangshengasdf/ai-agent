...
> **30 秒速读**：评估 Agent 和评估模型完全不同，你需要测行为轨迹（调了哪些工具、走了什么流程），而不只是测最终输出。本章教三种手段：行为测试、LLM-as-Judge 打分、回归测试套件。
> 
> **如果只记一件事**：只测输出会漏掉"碰巧蒙对但流程全错"的隐蔽 bug，行为测试才是 Agent 评估的灵魂。
...
| 错误 | 症状 | 解决 |
|------|------|------|
| 行为测试只 assert 最终输出 | Agent 碰巧猜对答案但没调工具，测试照样过 | 先断言 `tools_called` 包含期望工具，再看输出 |
| LLM-as-Judge 没给 rubric | Judge 打分随机，同一个回答跑三次分数不同 | prompt 里写明评分维度和每档含义，用 few-shot 锚定 |
| 回归测试用同一个模型当 Judge | Judge 偏爱自己风格的回答，分数虚高 | Judge 用不同于被评估 Agent 的模型 |
| 评估数据集只有"查天气"类用例 | 换了计算器工具后 Agent 崩了，回归套件测不出 | 每个工具至少 3 个用例，加边界和对抗样本 |
| 同一测试跑一次失败就报 FAIL | LLM 有随机性，偶尔抽风是正常的 | 同一用例跑 3-5 次，多数通过即算 PASS，或设 `temperature=0` |
